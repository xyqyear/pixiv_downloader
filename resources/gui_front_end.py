...
class FrontEnd(Process):
    ...
    def __init__(self, pipe, parent):
        ...
        self.window = gui.AppWindow("Pixiv Downloader")
        self.window.signal_input.connect(self.get_input)
        self.window.show()

    def run(self):
        self.input_info() # magic
        while True:
            pack = self.pipe.recv()
